package ru.teplicate.datasyncersmb.presentation.fragment.scan_fragment.view_model
...
import ru.teplicate.datasyncersmb.presentation.fragment.scan_fragment.AddressData
...
class ScanNetworkViewModel(
    ...
    fun testSmb(smbInfo: SmbInfoPresentation) {
        viewModelScope.launch(Dispatchers.IO) {
            val isConnected = try {
    ...
    fun setupConnectionData(smbInfo: SmbInfoPresentation) {
        this.smbInfo = smbInfo
    }

    fun makeArgs(): SmbInfoPresentation = requireNotNull(this.smbInfo)
